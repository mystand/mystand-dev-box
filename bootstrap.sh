...
(echo "deb http://repo.mongodb.org/apt/ubuntu "$(lsb_release -sc)"/mongodb-org/3.0 multiverse" | sudo tee /etc/apt/sources.list.d/mongodb.list) >/dev/null 2>&1

wget -qO - http://packages.elasticsearch.org/GPG-KEY-elasticsearch >/dev/null 2>&1 | sudo apt-key add -
(echo "deb http://packages.elasticsearch.org/elasticsearch/1.6/debian stable main" | sudo tee /etc/apt/sources.list.d/elasticsearch.list) >/dev/null 2>&1
